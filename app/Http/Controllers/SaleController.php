...
class SaleController extends Controller
{
    ...
    public function generatePDF($id)
    {
        ...
        // Si `sale_items` es null, aseguramos que sea un array vac�o para evitar errores
        $sale->sale_items = $sale->sale_items ?? [];
        $pdf = Pdf::loadView('sales.pdf', compact('sale', 'imagenQr', 'hashBase64'));
        return $pdf->stream('venta.pdf');
    }
    
    public function generatePDFNotaVenta($id)
    {
        $sale = Sale::with('saleItems.item', 'userRegister')->find($id);
        if (!$sale) {
            return abort(404, 'Venta no encontrada');
        ...
        $pdf = Pdf::loadView('sales.nota', compact('sale', 'imagenQr', 'hashBase64'))
                  ->setPaper([0, 0, 595.28, 420.94]); // tama�o personalizado vertical
    ...
    public function enviarSunat($id)
    {
        ...
        try {
            ...
            // Agregar cuotas_credito solo si no est� vac�o
            if (!empty($cuotas_credito)) {
                $data["cuotas_credito"] = $cuotas_credito;
            }

            // return response()->json($data);
            // Inicializaci�n de la instancia de ApiSunat
            $apiSunat = new ApiSunat('1');
            $apiSunat->setData($data);

            // Env�o de los datos y obtenci�n de la respuesta de la API
            ...
            if (!$respuestaSunat || !isset($respuestaSunat['estado']) || (isset($respuestaSunat['estado']) && !$respuestaSunat['estado'])) {
                ...
                $respuesta['message'] = "Error: Error en comunicaci�n con la API. respuesta: " . json_encode($respuestaSunat) . " -------------------- " . json_encode($data);
            } else {
                // Si la comunicaci�n fue exitosa, procesar el XML recibido
                ...
                // Verificar si el archivo se almacen� correctamente
        ...
        try {
            ...
            foreach ($salesItems as $salesItem) {
                if ($salesItem->item_type === Product::class) {
                    ...
                    if ($stock) {
                        $stock->quantity += $salesItem->quantity; // Revertir la reducci�n del stock
                        $stock->save();
                    }
                }
            }
    ...
    public function generateSerie($documentTypeId)
    {
        ...
        if (!isset($prefijos[$tipoDocumento->name])) {
            throw new \Exception('Tipo de documento no v�lido');
        }

        // FACTURA y BOLETA DE VENTA usan tres d�gitos (F001, B001), NOTA DE VENTA usa dos (NV01)
